...
import {
  SITE, conditions, getCondition, getSpecialty, questionsByTag,
} from "@/lib/data";
...
export default function ConditionPage({ params }) {
  const c = getCondition(params.condition);
  if (!c) notFound();
  const relatedQuestions = questionsByTag(c.slug).slice(0, 5);
  ...
  const medPage = {
    ...
    reviewedBy: { "@type": "Organization", name: c.reviewer },
  ...
  return (
    <div className="container">
      ...
      <div className="breadcrumb">
        <Link href="/">Home</Link> / <Link href="/conditions">Health guides</Link> / {c.name}
      </div>
